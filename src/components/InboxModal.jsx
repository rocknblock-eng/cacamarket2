...
export default function InboxModal({
  ...
  const formatTime = ts => {
    ...
    if (d.toDateString() === today.toDateString()) return d.toLocaleTimeString('pt-PT', {
      hour: '2-digit',
      minute: '2-digit'
    });
  ...
  return <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-pine-900 border border-pine-700 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl overflow-hidden flex flex-col" style={{
      maxHeight: '85vh'
    }}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-pine-700 shrink-0">
          <div className="flex items-center gap-2">
            ...
            {totalUnread > 0 && <span className="bg-blaze-500 text-pine-950 text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[20px] text-center">
                {totalUnread}
              </span>}
            {isAdmin && <span className="text-[10px] text-blaze-400 bg-blaze-500/10 border border-blaze-500/20 rounded-full px-2 py-0.5">
                Admin
              </span>}
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loading && conversations.length === 0 && <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          // Admin ve sempre "Comprador  Vendedor"
          // Utilizador normal ve o "outro"
          const other = isAdmin ? null : isBuyer ? conv.seller : conv.buyer;
          return <div key={conv.id} className="flex items-center border-b border-pine-800 hover:bg-pine-800/50 transition-colors">
                {/* Linha clicavel */}
                <button onClick={() => onOpenChat(conv)} className="flex-1 flex items-center gap-3 px-4 py-4 text-left min-w-0">
                  {/* Avatar */}
                  <div className="w-11 h-11 rounded-full bg-pine-700 border border-pine-600 flex items-center justify-center font-display font-bold text-bone-100 shrink-0 text-sm relative">
                    {isAdmin ? conv.buyer?.full_name?.charAt(0) || '?' : other?.full_name?.charAt(0) || '?'}
                    {conv.unread > 0 && <span className="absolute -top-0.5 -right-0.5 bg-blaze-500 text-pine-950 text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isAdmin ? <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                    ...
                    {/* Anuncio */}
                    <p className="text-xs text-bone-300/50 truncate mt-0.5">{'\u1f4e6 '}{conv.listings?.title || 'An\u00fancio removido'}
                    </p>

                    {/* Ultima mensagem + hora */}
                    <div className="flex items-center justify-between gap-2 mt-0.5">
                      {conv.lastMsg ? <p className={`text-xs truncate flex-1 ${conv.unread > 0 ? 'text-bone-200 font-medium' : 'text-bone-300/50'}`}>
                          {conv.lastMsg.sender_id === user.id ? 'Tu: ' : ''}
                          {conv.lastMsg.content}
                        </p> : <p className="text-xs text-bone-300/30 italic flex-1">Sem mensagens ainda</p>}
                ...
                {/* Botao apagar  so admin */}
                {isAdmin && <button onClick={e => handleDelete(e, conv.id)} disabled={deleting === conv.id} className="px-3 py-4 text-red-400/50 hover:text-red-400 transition-colors shrink-0 disabled:opacity-30" title="Apagar conversa">
                    <Trash2 size={16} />
                  </button>}
              </div>;
        })}
