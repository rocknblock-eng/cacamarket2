...
// ---------------------------------------------------------------------------
// PAYMENT_CONFIG
// Ativa/desativa metodos de pagamento aqui. Cada metodo fica "simulado"
// (nao processa pagamentos reais) ate o backend estar ligado a chaves
// secretas de Stripe/PayPal e a um IBAN/MB Way real.
// Ver README.md -> seccao "Proximo passo: backend de pagamentos".
//
// TABELA DE PRECOS (atual):
// - Particular: 1 gratis, depois 1 cada
// - Loja: 5 primeiros gratis, depois pacotes de 5 por 5
// - Admin: sempre gratis
//
// NOTA: Enquanto nenhuma data de lancamento for definida no painel admin,
// TODOS os utilizadores publicam de graca (modo teste/desenvolvimento).
// ---------------------------------------------------------------------------
export const PAYMENT_CONFIG = {
  stripeEnabled: true,
  paypalEnabled: true,
  mbwayEnabled: true,
  donationFeePercent: 0 // valor da comissao/donativo da plataforma (nao aplicavel agora)
};
export default function PaymentModal({
  ...
  function handlePay() {
    ...
    // SIMULACAO: aqui e onde, no futuro, se chamam os endpoints reais:
  ...
  return <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center">
      <div className="bg-pine-800 border border-pine-600 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm p-6 relative">
        ...
        {status === 'done' ? <div className="text-center py-6">
            ...
            <p className="text-bone-300/70 text-sm">{'Em produ\u00e7\u00e3o, aqui confirmar\u00edamos o pagamento real e notificar\u00edamos o vendedor.'}</p>
            <button onClick={onClose} className="mt-5 w-full bg-pine-700 hover:bg-blaze-500 hover:text-pine-950 text-bone-100 font-semibold text-sm py-2.5 rounded-lg transition-colors">
              Fechar
            </button>
          </div> : <>
            ...
            <div className="bg-pine-700/50 rounded-lg p-3 text-sm mb-4 space-y-1">
              ...
              {fee > 0 && <div className="flex justify-between text-bone-300/70">
            ...
            <button onClick={handlePay} disabled={status === 'processing'} className="w-full bg-blaze-500 hover:bg-blaze-600 disabled:opacity-60 transition-colors text-pine-950 font-semibold text-sm py-2.5 rounded-lg">
...
function PaymentOption({
  ...
  return <button onClick={() => onSelect(id)} className={`flex items-center gap-2 border rounded-lg px-3 py-2.5 text-sm text-left transition-colors ${selected ? 'border-blaze-500 bg-blaze-500/10 text-bone-100' : 'border-pine-600 text-bone-300 hover:border-pine-500'}`}>
      {icon}
      {label}
    </button>;
}