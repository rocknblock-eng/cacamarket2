...
function PhotoGallery({
  ...
  if (images.length === 0) {
    return <div className="aspect-[4/3] bg-pine-700 flex items-center justify-center text-bone-300/40 text-sm">
        Sem imagem
      </div>;
  }
  function prev() {
  ...
  return <>
      <div className="aspect-[4/3] bg-pine-700 relative overflow-hidden group">
        <img src={images[current]} alt={listing.title} className="w-full h-full object-cover cursor-zoom-in" onClick={() => open(current)} />
        ...
        {images.length > 1 && <div className="absolute bottom-2 right-2 bg-black/50 text-white text-xs px-2 py-0.5 rounded-full">
            {current + 1} / {images.length}
          </div>}
      </div>

      {images.length > 1 && <div className="flex gap-2 px-4 pt-2">
          {images.map((img, i) => <button key={i} onClick={() => open(i)} className={`relative w-14 h-14 rounded-md overflow-hidden border-2 transition-colors flex-shrink-0 group/thumb ${i === current ? 'border-blaze-400' : 'border-pine-600 opacity-60 hover:opacity-100'}`}>
      ...
      justifyContent: 'center'
          ...
          <img src={images[current]} alt={listing.title} style={{
        maxWidth: '95vw',
        maxHeight: '90vh',
        objectFit: 'contain'
  ...
  return <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-pine-800 border border-pine-600 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          {listing.condition && listing.condition !== '\u2014' && <div className="inline-flex w-fit bg-pine-700/50 text-bone-100 text-xs font-medium px-3 py-1.5 rounded-full">
              {listing.condition}
            </div>}

          {listing.description && <p className="text-bone-300 text-sm leading-relaxed">{listing.description}</p>}
          ...
          {isLoggedIn && (!isSeller || isAdmin) && <button onClick={() => onOpenChat(listing, seller)} className="w-full flex items-center justify-center gap-2 bg-blaze-500 hover:bg-blaze-400 text-pine-950 text-sm font-semibold py-3 rounded-lg transition-colors">
          ...
          {isLoggedIn && (!isSeller || isAdmin) && <>
              ...
              {showContact && <div className="flex flex-col gap-2 bg-pine-700/40 rounded-lg p-3">
                  {!hasContact && <p className="text-xs text-bone-300/60">{'Este vendedor ainda n\u00e3o adicionou informa\u00e7\u00e3o de contacto.'}</p>}
                  {seller.phone && <a href={whatsappLink(seller.phone, listing.title)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-[#25D366] hover:brightness-95 text-pine-950 text-sm font-semibold py-2.5 px-3 rounded-lg transition-all">
                      <MessageCircle size={16} />
                      WhatsApp
                    </a>}
                  {seller.phone && <a href={`tel:${seller.phone}`} className="flex items-center gap-2 bg-pine-700 hover:bg-pine-600 text-bone-100 text-sm font-semibold py-2.5 px-3 rounded-lg transition-colors">
                      <Phone size={16} />{'Ligar \u00b7 '}{seller.phone}
                    </a>}
                  {seller.email && <a href={emailLink(seller.email, listing.title)} className="flex items-center gap-2 bg-pine-700 hover:bg-pine-600 text-bone-100 text-sm font-semibold py-2.5 px-3 rounded-lg transition-colors">
                      <Mail size={16} />
                      Enviar email
                    </a>}
                </div>}
          ...
          {canDelete && <button onClick={() => onDelete(listing)} className="w-full flex items-center justify-center gap-2 text-red-400 hover:text-red-300 text-sm font-medium py-2 transition-colors">
              <Trash2 size={15} />{'Apagar an\u00fancio'}</button>}
        </div>
      </div>
    </div>;
}