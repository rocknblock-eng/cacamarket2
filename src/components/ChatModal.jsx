...
export default function ChatModal({ user, profile, listing, seller, conversationId: initialConvId, onClose }) {
  ...
  const initConversation = useCallback(async () => {
    ...
    const { data: created, error } = await supabase
      .from('conversations')
      .insert({ listing_id: listing.id, buyer_id: user.id, seller_id: seller.id })
      .select('id')
      .single()
  ...
  async function handleSend() {
    ...
    setSending(true)
    setInput('')
    await supabase.from('messages').insert({ conversation_id: conversationId, sender_id: user.id, content: text })
    setSending(false)
  }
  ...
  // Nome do vendedor — compatível com dados vindos do inbox (full_name) e do anúncio (name)
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-pine-900 border border-pine-700 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md flex flex-col shadow-2xl" style={{ height: '85vh', maxHeight: '600px' }}>
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-pine-700 shrink-0">
          ...
          {/* Menu de moderação (só admin) */}
          {isAdmin && (
            <div className="relative">
              <button
                onClick={() => setShowAdminMenu((v) => !v)}
                className="flex items-center gap-1 text-xs text-blaze-400 bg-blaze-500/10 border border-blaze-500/20 px-2 py-1 rounded-lg hover:bg-blaze-500/20 transition-colors"
              >
                <ShieldCheck size={13} />
                Admin
              </button>
              ...
              {showAdminMenu && (
                <div className="absolute right-0 top-full mt-1 bg-pine-800 border border-pine-600 rounded-xl shadow-xl z-10 min-w-[200px] overflow-hidden">
                  ...
                  {seller?.id && (
                    <button
                      onClick={() => handleBlock(seller.id, sellerName)}
                      disabled={blocking || blocked}
                      className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-red-400 hover:bg-red-400/10 transition-colors disabled:opacity-40 text-left"
                    >
                      <Ban size={15} />
                      Bloquear vendedor
                    </button>
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-red-400 hover:bg-red-400/10 transition-colors text-left border-t border-pine-700"
                  >
                    <Trash2 size={15} />
                    Apagar conversa
                  </button>
        ...
        {blocked && (
          <div className="px-4 py-2 bg-red-400/10 border-b border-red-400/20 text-xs text-red-400 flex items-center gap-2 shrink-0">
            <Ban size={13} />
            Utilizador bloqueado. Não pode enviar novas mensagens.
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4" onClick={() => setShowAdminMenu(false)}>
          ...
          {!loading && isSeller && !isAdmin && (
            <div className="flex items-center justify-center h-full">
              <p className="text-bone-300/40 text-sm text-center px-6">
                Não podes iniciar uma conversa sobre o teu próprio anúncio.
              </p>
            </div>
          )}
          ...
          {!loading && (isSeller === false || isAdmin || true) && Object.entries(grouped).map(([dateLabel, msgs]) => (
            <div key={dateLabel}>
              ...
              {msgs.map((msg) => (
                <div key={msg.id} className={`flex mb-2 ${isSelf(msg) ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[78%] flex flex-col ${isSelf(msg) ? 'items-end' : 'items-start'}`}>
                    {/* Admin vê sempre o nome de quem enviou */}
                    {(isAdmin || !isSelf(msg)) && (
                      <span className="text-xs text-bone-300/50 mb-0.5 mx-1">
                        {msg.profiles?.full_name}
                      </span>
                    )}
                    <div className={`px-3 py-2 rounded-2xl text-sm leading-relaxed break-words ${
                      isSelf(msg)
                        ? 'bg-blaze-500 text-pine-950 rounded-br-sm'
                        : 'bg-pine-700 text-bone-100 rounded-bl-sm'
                    }`}>
                      {msg.content}
                    </div>
                    <span className="text-[10px] text-bone-300/40 mt-0.5 mx-1">
                      {formatTime(msg.created_at)}
                      {isSelf(msg) && msg.read_at && (
                        <span className="ml-1 text-blaze-400/60">· lida</span>
                      )}
                    </span>
                  </div>
                </div>
          ...
          {!loading && !isSeller && !(isAdmin && !isSeller) && messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-2">
              <p className="text-bone-300/40 text-sm text-center px-6">
                Inicia a conversa com o vendedor sobre este anúncio.
              </p>
            </div>
          )}
          ...
          <div className="px-4 py-3 border-t border-pine-700 shrink-0">
            <div className="flex items-end gap-2">
              <textarea
                ...
                rows={1}
                maxLength={1000}
                className="flex-1 bg-pine-800 border border-pine-600 rounded-xl px-3 py-2.5 text-sm text-bone-100 placeholder:text-bone-300/40 outline-none focus:border-blaze-500 transition-colors resize-none"
              ...
              <button
                ...
                className="bg-blaze-500 hover:bg-blaze-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-pine-950 p-2.5 rounded-xl shrink-0"
              >
                <Send size={18} />
              </button>
            </div>
