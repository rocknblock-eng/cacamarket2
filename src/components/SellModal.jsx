...
export default function SellModal({ user, profile, isFreePeriod, onClose, onCreated, onPaymentRequired }) {
  ...
  function handlePhotosChange(e) {
    ...
    setError('')
    const newEntries = newFiles.map(f => ({ file: f, name: f.name, size: f.size }))
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-pine-800 border border-pine-600 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto relative">
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-3">
          ...
          {isFreePeriod ? (
            <p className="text-xs text-brass-400 -mt-2 mb-1">
              🎉 Período de lançamento — publicação grátis para todos.
            </p>
          ) : (
            <>
              {profile?.role === 'particular' && !profile.free_listing_used && (
                <p className="text-xs text-brass-400 -mt-2 mb-1">Este é o teu anúncio grátis.</p>
              )}
              {profile?.role !== 'admin' &&
                (profile?.role !== 'particular' || profile.free_listing_used) && (
                  <p className="text-xs text-brass-400 -mt-2 mb-1">
                    Este anúncio vai usar 1 dos teus donativos já feitos ({profile?.listing_credits ?? 0} disponíveis).
                  </p>
                )}
            </>
          )}
          ...
          <div className="flex flex-col gap-2">
            ...
            {photos.length > 0 && (
              <div className="flex flex-col gap-1.5">
                {photos.map((p, i) => (
                  <div key={i} className="flex items-center gap-2 bg-pine-700 rounded-lg px-3 py-2">
                    ...
                    <button
                      ...
                      className="text-bone-300/50 hover:text-red-400 shrink-0"
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
            ...
            {photos.length < MAX_PHOTOS && (
              <label className="h-16 bg-pine-700 border border-dashed border-pine-600 rounded-lg flex items-center justify-center cursor-pointer hover:border-blaze-500 transition-colors gap-2">
            ...
            {photos.length > 0 && (
                <span className="text-xs text-bone-300/40">{totalMB} / {MAX_TOTAL_MB} MB</span>
              )}
          ...
          <select
            ...
            className="bg-pine-700 border border-pine-600 rounded-lg px-3 py-2 text-sm text-bone-100 outline-none focus:border-blaze-500"
          >
            {CATEGORIES.map((c) => (
          ...
          <div className="flex gap-2">
            <input
              ...
              step="0.01"
              placeholder="Preço (€)"
              className="flex-1 bg-pine-700 border border-pine-600 rounded-lg px-3 py-2 text-sm text-bone-100 placeholder:text-bone-300/40 outline-none focus:border-blaze-500"
            />
            <select
              value={condition}
              onChange={(e) => setCondition(e.target.value)}
              className="flex-1 bg-pine-700 border border-pine-600 rounded-lg px-3 py-2 text-sm text-bone-100 outline-none focus:border-blaze-500"
            >
              <option>Novo</option>
          ...
          <textarea
            ...
            rows={3}
            className="bg-pine-700 border border-pine-600 rounded-lg px-3 py-2 text-sm text-bone-100 placeholder:text-bone-300/40 outline-none focus:border-blaze-500 resize-none"
          />
