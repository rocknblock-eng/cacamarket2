...
export default function EditListingModal({ user, listing, onClose, onUpdated }) {
  ...
  function handlePhotosChange(e) {
    ...
    setError('')
    setNewPhotos(prev => [...prev, ...newFiles.map(f => ({ file: f, name: f.name, size: f.size }))])
    e.target.value = ''
  }
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex gap-2 flex-wrap">
                {existingUrls.map((url, i) => (
                  <div key={i} className="relative w-20 h-20 rounded-lg overflow-hidden border border-pine-600">
                    <img src={url} alt={`Foto ${i + 1}`} className="w-full h-full object-cover" />
                    {i === 0 && (
                      <span className="absolute top-0 left-0 bg-black/60 text-[9px] text-brass-400 px-1">Principal</span>
                    )}
                    <button
                      type="button"
