...
export default function App() {
  ...
  const loadUnreadCount = useCallback(async () => {
    ...
    const {
      ...
      count: 'exact',
      head: true
    }).in('conversation_id', convs.map(c => c.id)).neq('sender_id', user.id).is('read_at', null);
    setUnreadCount(count || 0);
  }, [user]);
  useEffect(() => {
    if (!user) return;
    loadUnreadCount();
    const channel = supabase.channel('unread-badge').on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'messages'
    }, loadUnreadCount).on('postgres_changes', {
      event: 'UPDATE',
      schema: 'public',
      table: 'messages'
    }, loadUnreadCount).subscribe();
  ...
  const loadListings = useCallback(async () => {
    ...
    const now = new Date();
    const mappedListings = data.filter(row => !row.expires_at || new Date(row.expires_at) > now).map(row => ({
      id: row.id,
      title: row.title,
  ...
  useEffect(() => {
    ...
    if (paypalFlag === 'return' && orderId && user) {
      ...
      supabase.functions.invoke('paypal-capture-order', {
        body: {
          orderId
        }
      }).then(async ({
  ...
  async function handleDeleteListing(listing) {
    const confirmed = window.confirm('Tens a certeza que queres apagar este an\u00fancio? Esta a\u00e7\u00e3o n\u00e3o pode ser desfeita.');
    if (!confirmed) return;
  ...
  const allListings = useMemo(() => [...dbListings, ...LISTINGS.filter(l => !deletedDemoIds.includes(l.id)).map(l => ({
    ...l,
    source: 'demo'
  ...
  return <div className="min-h-screen bg-pine-900">
      ...
      {payWallOpen && profile && <PayWallModal profile={profile} onClose={() => setPayWallOpen(false)} />}
      {sellOpen && user && <SellModal user={user} profile={profile} isFreePeriod={isFreePeriod} onClose={() => setSellOpen(false)} onPaymentRequired={handlePaymentRequiredInSell} onCreated={() => {
      loadListings();
      refreshProfile(user.id);
    }} />}
      ...
      {editingListing && <EditListingModal user={user} listing={editingListing} onClose={() => setEditingListing(null)} onUpdated={() => {
      setEditingListing(null);
      loadListings();
    }} />}

      {lightbox && <div className="fixed inset-0 bg-black/95 flex items-center justify-center" style={{
      zIndex: 9999
    }} onClick={() => setLightbox(null)}>
          <img src={lightbox.images[lightbox.index]} alt="" className="max-w-[95vw] max-h-[90vh] object-contain" onClick={e => e.stopPropagation()} />
          <button onClick={() => setLightbox(null)} className="absolute top-4 right-4 text-white hover:text-orange-400 bg-black/50 rounded-full p-2">
            <X size={24} />
          </button>
      ...
      {inboxOpen && user && <InboxModal user={user} profile={profile} onClose={() => setInboxOpen(false)} onOpenChat={conv => {
      setInboxOpen(false);
      setActiveChat({
      ...
      {legalOpen && <LegalModal initialTab={legalOpen} onClose={() => setLegalOpen(null)} />}
      {activeChat && user && <ChatModal user={user} profile={profile} listing={activeChat.listing} seller={activeChat.seller} conversationId={activeChat.conversationId} onClose={() => {
      setActiveChat(null);
      loadUnreadCount();
    }} />}
      ...
      <footer className="border-t border-pine-700 mt-8 py-5 text-center">
        <div className="flex items-center justify-center gap-4 text-xs text-bone-300/40">
          <button onClick={() => setLegalOpen('terms')} className="hover:text-bone-200 transition-colors underline underline-offset-2">{'Termos e Condi\u00e7\u00f5es'}</button>
          <span>{'\u00b7'}</span>
          <button onClick={() => setLegalOpen('privacy')} className="hover:text-bone-200 transition-colors underline underline-offset-2">{'Pol\u00edtica de Privacidade'}</button>
          <span>{'\u00b7'}</span>
